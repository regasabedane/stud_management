...
if (isset($_POST['login'])) {
    ...
    if (!empty($username) && !empty($password)) {
        try {
            // Database irraa user barbaaduu
            $stmt = $conn->prepare("SELECT * FROM users WHERE username = :username");
            $stmt->execute([':username' => $username]);
            $user = $stmt->fetch(PDO::FETCH_ASSOC);

            // BAKKA KOODII ISAA KANAAN BAKKA BUUSSA:
            if ($user && password_verify($password, $user['password'])) {
                $_SESSION['user_id']    = $user['id'];
                $_SESSION['username']   = $user['username'];
                $_SESSION['role']       = $user['role'];       // Role (admin ykn student)
                $_SESSION['student_id'] = $user['student_id']; // ID barataa (yoo barataa ta'e)

                // Role irratti hundaa'uun gara fuula bakka sanaatti oofuu
                if ($user['role'] == 'admin') {
    ...
    <div class="card-body">
        ...
        <form action="login.php" method="POST" autocomplete="off">
            <div class="mb-3">
                ...
                <input type="text" name="username" class="form-control" required placeholder="Enter username" autocomplete="off">
            </div>
            <div class="mb-3">
